...
chop1col :: [String] -> ([String], [String])
chop1col rows =
  let go col rows =
        if all null rows || all (== ' ') (map head rows)
        then (map reverse col, map tail rows)
        else go (zipWith (:) (map head rows) col) (map tail rows)
   in go ["" | _ <- rows] rows
...
calc nums (op:ops) =
  let (col, nums') = chop1col nums
   in foldl1 op (map str2int col):calc nums' ops
